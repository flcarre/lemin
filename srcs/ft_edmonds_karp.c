...
/*   Updated: 2020/03/09 16:47:30 by lutsiara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int new_flow)
{
	...
	while (v_room)
	{
		...
		var->residual_matrix[i_u + 1][i_v + 1].value -= new_flow;
		var->residual_matrix[i_v + 1][i_u + 1].value += new_flow;
...
t_ctn			*ft_edmonds_karp(t_var *var)
{
	...
	while ((new_flow = ft_ek_bfs(var, s, t)))
	{
		ft_init_visited(var);
		ft_del_links(&var->file);
		ft_update_residual_matrix(var, var->end, new_flow);
	}
